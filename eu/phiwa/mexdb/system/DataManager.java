...
import eu.phiwa.mexdb.exception.EmptyListException;
import eu.phiwa.mexdb.exception.NoSuchIndexException;
import eu.phiwa.mexdb.file.IOManager;
...
public class DataManager {
	 ...
	 */
	public void setList(String index, String key, List<String> list)
			throws EmptyListException {

		if (list.isEmpty())
			throw new EmptyListException(
					"Empty list passed on method call setList()");
	...
	public void replaceIndex(String index, String replace)
		...
		if (!content.containsKey(indexl))
			throw new NoSuchIndexException("Index: \"" + indexl
					+ "\" does not exist!");
	...
	public int getInt(String index, String key) {
		String mkey = key.toLowerCase().trim();
		return Integer.valueOf(getSharpKeySet(index.toLowerCase(), mkey).get(
				mkey));
	}
	...
	public double getDouble(String index, String key) {
		String mkey = key.toLowerCase().trim();
		return Double.valueOf(getSharpKeySet(index.toLowerCase(), mkey).get(
				mkey));
	}
	...
	public long getLong(String index, String key) {
		String mkey = key.toLowerCase().trim();
		return Long
				.valueOf(getSharpKeySet(index.toLowerCase(), mkey).get(mkey));
	}
	...
	public float getFloat(String index, String key) {
		String mkey = key.toLowerCase().trim();
		return Float.valueOf(getSharpKeySet(index.toLowerCase(), mkey)
				.get(mkey));
	}
	...
	public boolean getBoolean(String index, String key) {
		String mkey = key.toLowerCase().trim();
		return Boolean.valueOf(getSharpKeySet(index.toLowerCase(), mkey).get(
				mkey));
	}
	 ...
	 */
	private LinkedHashMap<String, String> getSharpKeySet(String index,
			String key) {
		...
		for (int counter = 0; counter < seg.length; counter++) {
			...
			argsmap.put(shorterseg[0], shorterseg.length == 2 ? shorterseg[1]
					: null);
	...
	public boolean hasKey(String index, String key) {
		if (getKeySet(index.toLowerCase().trim()).containsKey(
	 ...
	 */
	private void refreshContent(String index,
			LinkedHashMap<String, String> keyset) {
		...
		for (Map.Entry<String, String> entry : keyset.entrySet()) {
			if (!first) {
				build.append(entry.getKey().concat(":")
						.concat(entry.getValue()));
				first = true;
			} else
				build.append(" ".concat(entry.getKey()).concat(":")
						.concat(entry.getValue()));
		}
