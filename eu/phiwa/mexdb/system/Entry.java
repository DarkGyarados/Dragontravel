...
public class Entry {
	...
	public void addValue(Object key, Object keyvalue) {
		...
		if (first) {
			valuebuilder.append(keya.trim().toLowerCase().concat(":")
					.concat(cipher(valuea)));
			first = false;
		} else
			valuebuilder.append(" ".concat(keya.trim().toLowerCase())
					.concat(":").concat(cipher(valuea)));
	}
	 ...
	 */
	public void addList(Object key, List<String> list)
			throws EmptyListException {

		if (list.isEmpty())
			throw new EmptyListException(
					"Empty list passed on method call addList()");
		...
		if (first) {
			valuebuilder.append(keya.trim().toLowerCase().concat(":")
					.concat(cipher(builder.toString())));
			first = false;
		} else
			valuebuilder.append(" ".concat(keya.trim().toLowerCase())
					.concat(":").concat(cipher(builder.toString())));
	}
